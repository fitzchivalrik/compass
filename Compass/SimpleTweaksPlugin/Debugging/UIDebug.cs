...
namespace SimpleTweaksPlugin.Debugging {
    ...
    public unsafe class UIDebug  {

        public UIDebug(Compass.Compass plugin)
        {
            Plugin = plugin;
        }

        private Compass.Compass Plugin { get; set; }
        ...
        public delegate AtkStage* GetAtkStageSingleton();
        private GetAtkStageSingleton? getAtkStageSingleton;

        public GetAtkStageSingleton GetAtkStageSingletonFunc
        {
            get
            {
                if (getAtkStageSingleton != null) return getAtkStageSingleton;
        ...
        private bool FreeExclusiveDraw() {
            ...
            var dalamud = (Dalamud.Dalamud) typeof(DalamudPluginInterface).GetField("dalamud", BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(Plugin.PluginInterface);
        ...
        public void DrawUnitBase(AtkUnitBase* atkUnitBase) {
        ...
        public void PrintNode(AtkResNode* node, bool printSiblings = true, string treePrefix = "", bool textOnly = false)
        {
            if (node == null)
                return;
        ...
        public void PrintSimpleNode(AtkResNode* node, string treePrefix, bool textOnly = false)
        {
            bool popped = false;
        ...
        private void PrintResNode(AtkResNode* node)
        {
            ...
            ImGui.DragFloat($"X##{(ulong) node:X}", ref node->X);
            ImGui.SameLine();
            ImGui.SetNextItemWidth(50f);
            ImGui.DragFloat($"Y##{(ulong) node:X}", ref node->Y);
            ImGui.SameLine();
            ImGui.Text(
                ...
                for (var j = 0; j < unitManager->Count && headerOpen; j++) {
                    ...
                    if (headerOpen) {
                        ...
                        if (ImGui.Selectable($"{name}##list{i}-{(ulong) unitBase:X}_{j}", selectedUnitBase == unitBase)) {
                            ...
                            Plugin.PluginInterface.SavePluginConfig(Plugin.PluginConfig);
